    ...
    <main>
      ...
      <section id="screen-home" class="screen active">
        ...
        <div class="search-box">
          <span>🔍</span>
          <input type="text" id="search-input" placeholder="지역, 가게, 키워드 검색 (데모)" />
        </div>
      ...
      <section id="screen-signup" class="screen">
        ...
        <div class="input-group">
          <label>아이디</label>
          <input type="text" id="signup-id" placeholder="영문·숫자 조합" />
        </div>
        <div class="input-group">
          <label>비밀번호</label>
          <input type="password" id="signup-pw" placeholder="8자 이상" />
        </div>
        <div class="input-group">
          <label>이메일</label>
      ...
      <section id="screen-detail" class="screen">
        ...
        <button class="btn secondary" id="btn-goto-myjobs">내 지원 내역 보기</button>
...
<section id="screen-apply" class="screen">
  ...
  <div class="card" id="apply-resume-card" style="display:none;">
    ...
    <div class="mini" id="apply-resume-summary" style="margin-top:10px;"></div>

    <div class="row" style="margin-top:10px; gap:8px;">
      <button type="button" class="btn primary flex1" id="btn-apply-with-resume">
        이력서로 지원하기
      </button>
      <button type="button" class="btn outline flex1" id="btn-apply-simple">
        간단 지원하기
      </button>
    </div>
  </div>
  ...
  <div class="card" id="apply-no-resume-card" style="display:none;">
    ...
    <div class="row" style="margin-top:10px; gap:8px;">
      ...
      <button type="button" class="btn outline flex1" id="btn-apply-simple2">
        이력서 없이 지원
      </button>
    </div>
  </div>
  ...
  <div id="apply-simple-form" style="display:none; margin-top:16px;">
    <div class="input-group">
      <label>이름</label>
      <input type="text" id="apply-name" placeholder="실명 입력" />
    </div>

    <div class="input-group">
      <label>연락처</label>
      <input type="text" id="apply-phone" placeholder="010-0000-0000" />
    </div>

    <div class="input-group">
      <label>한 줄 소개</label>
      <input type="text" id="apply-intro"
             placeholder="예) 책임감 있게 성실히 일하겠습니다." />
    </div>

    <div class="input-group">
      <label>자기소개 / 경력</label>
      <textarea id="apply-desc"
                placeholder="간단한 경력과 자기소개를 적어주세요."></textarea>
    </div>

    <button class="btn primary" id="btn-submit-apply">
      지원서 보내기
    </button>
  </div>
...
<section id="screen-post" class="screen">
  ...
  <div class="input-group">
    <label>가게 이름</label>
    <input type="text" id="post-company" placeholder="예) 감성카페 오동도점" />
  </div>

  <div class="input-group">
    <label>공고 제목</label>
    <input type="text" id="post-title" placeholder="예) 카페 알바 (바리스타)" />
  </div>
...
<div class="input-group">
  ...
  <div class="row" id="pay-amount-row">
    <input type="number" id="post-pay" placeholder="예) 10000" min="9860" />
    <span class="suffix">원</span>
  </div>
  <div class="helper" id="pay-helper">최저시급 10,030원 미만은 설정할 수 없어요.</div>
</div>
...
<div class="input-group">
  ...
  <div class="helper" id="days-helper">요일을 선택하거나 “협의”를 선택할 수 있어요.</div>
</div>
...
<div class="input-group">
  ...
  <div class="row" id="time-select-row">
    <select id="post-start"></select>
    <span class="dash">~</span>
    <select id="post-end"></select>
    <select id="time-step" class="step">
  ...
  <div class="row" id="time-custom-row" style="display:none;">
    <input type="time" id="post-start-custom" step="900" />
    <span class="dash">~</span>
    <input type="time" id="post-end-custom" step="900" />
  </div>

  <div class="helper" id="time-helper">시간은 선택/직접입력/협의 중 하나로 설정해요.</div>
</div>
  ...
  <div class="input-group">
    ...
    <div class="row">
      ...
      <select id="post-district">
        <option value="">구/군</option>
        <option value="서구">서구</option>
        <option value="남구">남구</option>
      </select>
    </div>
    <input type="text" id="post-address" placeholder="상세 위치(선택) 예) 충장로 1가" />
  </div>

  <!-- 설명 -->
  <div class="input-group">
    <label>간단 설명</label>
    <textarea id="post-desc" maxlength="500" placeholder="업무 내용, 우대사항 등을 적어주세요. (최대 500자)"></textarea>
  </div>
...
<div class="row">
  <div class="input-group flex1">
    <label>이름</label>
    <input type="text" id="resume-name" readonly />
  </div>

  <div class="input-group flex1">
    <label>나이</label>
    <input type="text" id="resume-age" readonly />
  </div>
</div>

<div class="input-group">
  <label>전화번호</label>
  <input type="text" id="resume-phone" readonly />
</div>
  ...
  <div class="input-group">
    <label>희망 시급</label>
    <div class="row">
      <input type="number" id="resume-wage" name="desiredWage" placeholder="예) 12000" min="0" />
      <label class="inline">
...
<div class="input-group">
  ...
  <div class="segmented">
    <button type="button" class="seg-btn" data-mode="negotiable" id="resume-days-neg">
      요일 협의
    </button>
  </div>
</div>
  ...
  <div class="input-group">
    <label>근무 시간</label>
    <div class="row">
      <input type="time" id="resume-time-start" name="timeStart" />
      <span class="tilde">~</span>
      <input type="time" id="resume-time-end" name="timeEnd" />
      <label class="inline">
        <input type="checkbox" id="resume-time-neg" name="timeNegotiable" />
        시간 협의
      </label>
    </div>
  </div>
  ...
  <div class="block-title">
    경력
    <button type="button" class="btn small" id="btn-add-exp">+ 경력 추가</button>
  </div>
  ...
  <div id="exp-list">
    ...
    <div class="exp-item">
      <div class="row">
        <div class="input-group flex1">
          <label>근무처(가게명)</label>
          <input type="text" class="exp-store" name="expStore" placeholder="예) 스타카페 광주점" />
        </div>
        <div class="input-group flex1">
          <label>업종</label>
          <input type="text" class="exp-industry" name="expIndustry" placeholder="예) 카페, 편의점" />
        </div>
      </div>
      ...
      <div class="row">
        <div class="input-group flex1">
          <label>근무 기간</label>
          <input type="text" class="exp-period" name="expPeriod" placeholder="예) 2024.03 ~ 2024.09 / 6개월" />
        </div>
        <div class="input-group flex1">
          <label>역할</label>
          <input type="text" class="exp-role" name="expRole" placeholder="예) 음료 제조, 서빙, 마감" />
        </div>
      </div>
...
<div class="input-group">
  ...
  <div class="segmented" id="resume-skills">
    <button type="button" class="seg-btn" data-skill="POS">POS 사용</button>
...
<div class="screen-title">
    <button class="btn primary" id="btn-back-applicants" data-goto="applicants">← 목록</button>
</div>
  <!-- 저장 -->
  <button class="btn primary" id="btn-save-resume">이력서 저장하기</button>
      ...
      <section id="screen-profile" class="screen">
        ...
        <div class="input-group">
          <label>닉네임</label>
          <input type="text" id="profile-nick" placeholder="예) 열정알바생" />
        </div>
        <div class="input-group">
          <label>사는 지역</label>
          <input type="text" id="profile-area" placeholder="예) 광주 동구" />
        </div>
        <div class="input-group">
          <label>한 줄 소개</label>
          <input type="text" id="profile-intro" placeholder="예) 웃으면서 일하는 알바생입니다." />
        </div>
...
<nav class="bottom-nav">
  ...
  <!-- 알바생 전용 -->
  <div class="nav-item" data-target="myjobs" id="nav-myjobs" style="display:none;">
    <span>📌</span><div>지원내역</div>
  </div>

  <div class="nav-item" data-target="resume" id="nav-resume"style="display:none;">
    <span>📄</span><div>이력서</div>
  </div>

  <!-- ✅ 사장님 전용: 공고관리 -->
  <div class="nav-item" data-target="owner" id="nav-owner"style="display:none;">
    <span>🧾</span><div>공고관리</div>
  </div>
